...
vec3 bringback(vec3 p){
    ...
    const int n_gens = 8;
    vec3 generators[8];
    float numb = .86602540378443864676;
    generators[6] = vec3(p.x+full_size*numb, p.y -full_size/2., p.z);
    generators[7] = vec3(p.x-full_size*numb, p.y +full_size/2., p.z);
    generators[5] = vec3(p.x, p.y, p.z-full_size);
    generators[4] = vec3(p.x, p.y, p.z+full_size);
    generators[3] = vec3(p.x-full_size*numb, p.y -full_size/2., p.z);
    generators[2] = vec3(p.x+full_size*numb, p.y +full_size/2., p.z);
