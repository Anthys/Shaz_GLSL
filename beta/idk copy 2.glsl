...
void main(){
  ...
  float n = warp(coord.xy, .003+0.001 , 615.-400.);
  n = warp(coord.xy, .003, 615.);
  i = i - int(n);
  vec2 supercool = int2pos(i, u_resolution);
  supercool = vec2(int(mod(float((coord.x-n)),u_resolution.x)),int(coord.y-floor(float(n)/u_resolution.x)));
