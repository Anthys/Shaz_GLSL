...
float patternD( in vec2 p ,float t)
{
    vec2 q = vec2( fbm( p + vec2(0.0,0.0) ,t),
                   fbm( p + vec2(5.2,1.3) ,t) );
...
void main(){
  ...
  //mid = vec2(-2.);
  vec2 new = (st -mid)*2.;
  float dist = sqrt(new.x*new.x+new.y*new.y);
  float n = (patternA(vec2(dist))+patternB(st/7.)*2.)/(2.);
  //n = warp(coord.xy, .001,610.)/610.;
  //st = (st+n)/2.+1.-u_time/10.;
  vec2 newpos = (st+n)/2.;
  vec4 col1 = texture2D(u_texture_0, st);
  vec4 col2 = texture2D(u_texture_0, newpos);
  gl_FragColor = texture2D(u_texture_0, newpos);
  vec3 final_col = mix(col1.rgb, col2.rgb,1.);
