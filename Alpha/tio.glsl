...
void main() {
    ...
    float pct = 0.;
    vec2 grid = it*50.*it.x;
    //grid = grid*ceil(grid.y/5.);
    grid = grid+cos(ceil(grid.x)); 
    float data = fract(ceil(grid.x)/2.)*2.;
    float data2 = fract(ceil(grid.y)/2.)*2.;
    vec4 final_col = vec4(data,data2,1.,1.);
    gl_FragColor = final_col;



    return;
    float ss = ceil(st.y*10.);
    ss=1.;
    //if (st.y>.5){ss=2.;}
    vec2 grid2 = fract(grid/ss);
    float truc = 0.;
    if (grid2.x < grid2.y){
        truc = .5;
    }
    vec2 grid3 = floor(grid/ss);
    float tri_val = snoise((grid3-truc))*0.5+0.5;
    grid3.x;
    vec2 bst = (st-vec2(0.5,0.5))*2.;
    vec2 center = vec2(0.5,0.5);
    //bst -= center;
    float dist = sqrt(bst.x*bst.x+bst.y*bst.y);
    float dist1 = grid3.y*.04;
    float dist2 = (57.-grid3.y)*.04;
    pct = step(snoise((grid3-truc))*0.5+0.5,.8);
    pct = snoise((grid3-truc))*0.5+0.5;
    //pct /= dist;
    pct = max(step(1.-pct/dist1, 0.2), step(1.-pct/dist2, 0.2));
    ...
    gl_FragColor = final_col;
    grid3 /=50.;
    gl_FragColor = vec4(grid3.x,grid3.x,0.,1.);
