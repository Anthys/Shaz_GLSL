...
vec2 rotate(vec2 p, float a){
  vec2 t = vec2(p.x*cos(a)-sin(a)*p.y, p.x*sin(a)+cos(a)*p.y);
  return t;
}
void main(){
  ...
  float seuil = .4;
  float a = centered.x+centered.y*2.;
  l=snoise(vec3((l)*2.,u_time, st.x));
  st = rotate(st-vec2(.5),l)+vec2(.5);
  float ll = 3.*(abs(sin(u_time))-.2)*(-length(centered));
  st = st-centered*ll;
